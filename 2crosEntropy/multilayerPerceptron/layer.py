...
""" Fully connected layer.
    outputs = inputs @ weight + bias or
    outputs = dot(inputs, weight) + bias
...
# change colovution to matrix multiplication via im2col trick, which is faster than naive convolution implementation.
# graphical to columns
"""
img = np.array([
    [[1], [2], [3], [4]],
    [[5], [6], [7], [8]],
...
│ D │ E │ F │ G │
└───┴───┴───┴───┘
extract patches with kernel size 2 * 2 and stride 1, get 9 patches:
┌───┬───┐ ┌───┬───┐ ┌───┬───┐
│ 1 │ 2 │ │ 2 │ 3 │ │ 3 │ 4 │
├───┼───┤ ├───┼───┤ ├───┼───┤
│ 5 │ 6 │ │ 6 │ 7 │ │ 7 │ 8 │
...
def get_padding_2d(in_shape, k_shape, mode):
    """ input = 5 * 5, kernel = 3 * 3, stride = 1
        SAME padding: 5 - 1 + 3 - 5 = 2, half = 1, padding = (1, 1) -> left 1, right 1
        input = 5 * 5, kernel = 4 * 4, stride = 1
        SAME padding: 5 - 1 + 4 - 5 = 3, half = 1, padding = (1, 2) -> left 1, right 2 not symmetric
    """
