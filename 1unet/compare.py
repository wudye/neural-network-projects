import os
os.environ['CUDA_VISIBLE_DEVICES'] = '' #指定GPU编
import torch
import numpy as np
import unet
...
for i in range(len(y_train_label)):
    if y_train_label[i] <= 2:                    #为了加速演示作者只对数据集中的小于2的数字，也就是0和1进行运行，读者可以自行增加训练个数
        x_train_batch.append(x_train[i])
"""
x_train = np.reshape(x_train, [-1, 1, 28, 28])  #修正数据输入维度：([30596, 28, 28])
x_train /= 512.
...
for epoch in range(5):
    ...
    if epoch%4 == 0 and epoch != 0:                            #每30轮保存一次模型
        torch.save(model.state_dict(),"./1unet.pth")
